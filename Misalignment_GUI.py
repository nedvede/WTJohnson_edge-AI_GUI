...
class ImageCaptureGUI:
    def __init__(self, master):
        ...
        self.manual_check_button = Checkbutton(master, text="Manual Inspect", variable=self.manual_check, command = self.update_buttons, width=20, height=1)
        self.manual_check_button.grid(row=4, column=0)

        self.check_button = Checkbutton(master, text="Misalignment Detection", variable=self.process_check, command = self.update_buttons, width=20, height=1)
    ...
    def update_image(self):
        # if self.manual_check.get() or self.process_check.get():
        ...
        if self.manual_check.get() == 1:
            self.manual_inspect_pattern()

        elif self.process_check.get() == 1:
    ...
    def close_orginal_open_processing(self):
        if self.isRunning:
            ...
            self.master.after(300, self.update_image)  # update every second
    ...
    def stop_system(self):
        ...
        self.status_indicator2.config(text="STop", bg="gray")
        self.isRunning = False
    ...
    def update_buttons(self):
        if self.manual_check.get() and self.process_check.get():
            messagebox.showerror("Error", "Please select only one pattern.")

        elif self.manual_check.get():
            self.accept_button.config(state='normal')
            self.reject_button.config(state='normal')
            self.start_button.config(state='normal')
            self.stop_button.config(state='normal')
            self.option_menu.config(state="disabled")
        elif self.process_check.get():
            self.accept_button.config(state='normal')
            self.reject_button.config(state='normal')
            self.start_button.config(state='normal')
            self.stop_button.config(state='normal')
